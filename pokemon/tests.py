...
class PokemonTestCase(TestCase):
    def setUp(self):
        ...
        self.species1 = Species.objects.create(name="snorlax", type1=self.type1, id=1, hp=50,attack=50,
                                               special_attack=50, defense= 50, special_defense=50, speed=50)
        self.species2 = Species.objects.create(name="ho-oh", type1=self.type2, id=2, hp=50,attack=50,
                                               special_attack=50, defense= 50, special_defense=50, speed=50)
    ...
    def test_tackle(self):
        ...
        move = Move.objects.create(name="tackle", power=40, priority=0,
                            accuracy=100, pp=35, type=self.type1, damage_class="PH")
    ...
    def test_hyperbeam(self):
        ...
        move = Move.objects.create(name="hyper-beam", power=150, priority=0,
                            accuracy=90, pp=5, type=self.type1, damage_class="SP")
        damage = move.max_damage(attacker=self.attacker, defender=self.defender)
        self.assertEqual(damage, 52)

    def test_futuresight(self):
        "Testing implementation of future sight move"
        move = Move.objects.create(name="future-sight", power=120, priority=0,
                                   accuracy=100, pp=10, type=self.type3, damage_class="SP")
        damage = move.max_damage(attacker=self.attacker, defender=self.defender)
        self.assertEqual(damage, 28)